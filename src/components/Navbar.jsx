...
const Navbar = () => {
    ...
    return (
            <div className={`fixed ${isScrollingUp ? 'transform translate-y-0' : 'transform -translate-y-full'} top-0 bg-white/30 backdrop-blur-lg border-b border-white/10  z-[999] w-full px-8 py-8 flex justify-between items-center  text-black`}>
            ...
            <div className="links flex gap-10">
                {["Services","Our Work","About Us","Insights","Contact Us"].map((item,index) => {
                    return <a key={index} className={`text-lg capitalize font-normal ${index===4 && "ml-32"} `}>{item}</a>
                })}
            </div>
        </div>
    )}
